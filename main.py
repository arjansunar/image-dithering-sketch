...
def display(img_path,cv2_img_arr):
    ...
    inp_label.pack()
    image_label =  Label(b1,image=input_img)
    ...
    output_image_label = Label(b2,image=out_img)
...
def upload_file():
    ...
    if filename:
        ...
        quanBtn= tk.Button(app,textvariable=quanBtn_text,command=lambda: display(filename, quantize_img(filename, 3))
        ...
        dethBtn= tk.Button(app,textvariable=dethBtn_text,command=lambda:display(filename, image_to_dithering(filename))
                    ,font="Raleway",bg="black",fg="white",width=15,
                        bd=0, highlightthickness=0, relief='ridge')
        ...
        sketchBtn= tk.Button(app,textvariable=sketchBtn_text,command=lambda:display(filename, image_to_sketch(filename))
                    ,font="Raleway",bg="black",fg="white",width=15,
                        bd=0, highlightthickness=0, relief='ridge')
...
upBtn= tk.Button(app,textvariable=uploadBtn_text,command= disable_btn
               ,font="Raleway",bg="black",fg="white",width=15,
                bd=0, highlightthickness=0, relief='ridge')
